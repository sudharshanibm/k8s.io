...
resource "ibm_pi_key" "sshkey" {
  ...
  pi_ssh_key           = module.secrets_manager[0].k8s_powervs_ssh_public_key
  pi_cloud_instance_id = ibm_pi_workspace.housekeeping[0].id
}
...
module "powervs_workspace_us_south" {
  ...
  depends_on        = [ibm_pi_workspace.housekeeping[0]]
  count  = var.target_architecture == "ppcle" ? 1 : 0
  source            = "../modules/pvs_workspace"
  datacenter        = "us-south"
